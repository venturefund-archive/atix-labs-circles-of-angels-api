...
const createActivity = fastify => async (req, reply) => {
  ...
  try {
    ...
    fastify.log.error('[Activity Routes] :: Error creating activity: ', error);
    reply.status(500).send({ error: 'Error creating activity' });
  }
};

const updateActivity = fastify => async (req, reply) => {
  const { activityService } = apiHelper.helper.services;
  fastify.log.info(
  ...
  try {
    ...
    fastify.log.error('[Activity Routes] :: Error updating activity: ', error);
    reply.status(500).send({ error: 'Error updating activity' });
  }
};

const deleteActivity = fastify => async (request, reply) => {
  ...
  try {
    const deleted = await activityService.deleteActivity(id, milestoneService);
...
const uploadEvidence = fastify => async (req, reply) => {
  const { activityService } = apiHelper.helper.services;
  const { id } = req.params;
  fastify.log.info(
  ...
  try {
    ...
    fastify.log.error('[Activity Routes] :: Error uploading evidences:', error);
    reply.status(500).send({ error: 'Error uploading evidences' });
  }
};

const deleteEvidence = fastify => async (request, reply) => {
  const { activityService } = apiHelper.helper.services;
...
const getActivity = fastify => async (request, reply) => {
  const { activityService } = apiHelper.helper.services;
  const { id } = request.params;
...
const assignOracle = fastify => async (request, reply) => {
  const { activityService } = apiHelper.helper.services;
...
const unassignOracle = fastify => async (request, reply) => {
  const { activityService } = apiHelper.helper.services;
  const { id } = request.params;
  fastify.log.info(
...
const downloadEvidence = fastify => async (request, reply) => {
  const { activityService } = apiHelper.helper.services;
  const { activityId, evidenceId, fileType } = request.params;
  fastify.log.info(
...
const completeActivity = fastify => async (request, reply) => {
  const { activityService, milestoneService } = apiHelper.helper.services;
...
module.exports = fastify => ({
  createActivity: createActivity(fastify),
  updateActivity: updateActivity(fastify),
  deleteActivity: deleteActivity(fastify),
  uploadEvidence: uploadEvidence(fastify),
  deleteEvidence: deleteEvidence(fastify),
  getActivity: getActivity(fastify),
  assignOracle: assignOracle(fastify),
  unassignOracle: unassignOracle(fastify),
  downloadEvidence: downloadEvidence(fastify),
  completeActivity: completeActivity(fastify)
});
