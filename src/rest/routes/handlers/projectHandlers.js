...
const createProject = fastify => async (req, reply) => {
...
const getProjects = fastify => async (request, reply) => {
  const { projectService } = apiHelper.helper.services;
...
const getActiveProjects = fastify => async (request, reply) => {
  const { projectService } = apiHelper.helper.services;
  fastify.log.info('[Project Routes] :: Getting projects');
  try {
...
const getProject = fastify => async (request, reply) => {
  const { projectService } = apiHelper.helper.services;
...
const updateStatus = fastify => async (request, reply) => {
  const { projectService } = apiHelper.helper.services;
...
const deleteProject = fastify => async (request, reply) => {
  const { projectService } = apiHelper.helper.services;
...
const getProjectMilestones = fastify => async (request, reply) => {
  const { projectService } = apiHelper.helper.services;
...
const downloadMilestonesTemplate = fastify => async (request, reply) => {
  const { projectService } = apiHelper.helper.services;
...
const downloadProposalTemplate = fastify => async (request, reply) => {
  const { projectService } = apiHelper.helper.services;
  fastify.log.info(
...
const getMilestonesFile = fastify => async (request, reply) => {
  const { projectService } = apiHelper.helper.services;
  const { projectId } = request.params;
  fastify.log.info(
    `[Project Routes] :: Getting project milestones of project ${projectId}`
...
const uploadAgreement = fastify => async (request, reply) => {
  const { projectService } = apiHelper.helper.services;
...
const downloadAgreement = fastify => async (request, reply) => {
  const { projectService } = apiHelper.helper.services;
...
const downloadProposal = fastify => async (request, reply) => {
  const { projectService } = apiHelper.helper.services;
...
const updateProject = fastify => async (req, reply) => {
  const { projectService } = apiHelper.helper.services;
  fastify.log.info(
...
const getTotalFunded = fastify => async (request, reply) => {
  const { projectService } = apiHelper.helper.services;
...
const startProject = fastify => async (request, reply) => {
  const { projectService } = apiHelper.helper.services;
  const { id } = request.params;
...
const getProjectsByOracle = fastify => async (request, reply) => {
  const { projectService } = apiHelper.helper.services;
  const { id } = request.params;
...
const uploadExperience = fastify => async (request, reply) => {
  const { projectService } = apiHelper.helper.services;
  const { id } = request.params;
...
const getExperiences = fastify => async (request, reply) => {
  const { projectService } = apiHelper.helper.services;
  const { id } = request.params;
  fastify.log.info(
...
module.exports = fastify => ({
  createProject: createProject(fastify),
  getProjects: getProjects(fastify),
  getActiveProjects: getActiveProjects(fastify),
  getProject: getProject(fastify),
  updateStatus: updateStatus(fastify),
  deleteProject: deleteProject(fastify),
  getProjectMilestones: getProjectMilestones(fastify),
  downloadMilestonesTemplate: downloadMilestonesTemplate(fastify),
  downloadProposalTemplate: downloadProposalTemplate(fastify),
  getMilestonesFile: getMilestonesFile(fastify),
  uploadAgreement: uploadAgreement(fastify),
  downloadAgreement: downloadAgreement(fastify),
  downloadProposal: downloadProposal(fastify),
  updateProject: updateProject(fastify),
  getTotalFunded: getTotalFunded(fastify),
  startProject: startProject(fastify),
  getProjectsByOracle: getProjectsByOracle(fastify),
  uploadExperience: uploadExperience(fastify),
  getExperiences: getExperiences(fastify)
});
